## Settings
# Set ZSH_DOTENV_ENABLED=true in your environment to enable auto-sourcing .env files
# For example, add to ~/.zshenv or a private module:
#   export ZSH_DOTENV_ENABLED=true

[[ "$ZSH_DOTENV_ENABLED" == "true" ]] || return

# Filename of the dotenv file to look for
: ${ZSH_DOTENV_FILE:=.env}

# Path to the file containing allowed paths
: ${ZSH_DOTENV_ALLOWED_LIST:="${ZSH_DATA_DIR:-$HOME/.cache}/dotenv-allowed.list"}
: ${ZSH_DOTENV_DISALLOWED_LIST:="${ZSH_DATA_DIR:-$HOME/.cache}/dotenv-disallowed.list"}
